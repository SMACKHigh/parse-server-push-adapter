...
describe('ParsePushAdapter', () => {
  ...
  it('can get valid push types', done => {
    ...
    expect(parsePushAdapter.getValidPushTypes()).toEqual([
      'ios',
      'osx',
      'tvos',
      'android',
  ...
  it('can classify installation', done => {
    ...
    var deviceMap = ParsePushAdapter.classifyInstallations(
      installations,
      validPushTypes,
    );
    expect(deviceMap['android']).toEqual([
  ...
  it('reports properly results', done => {
    ...
    var installations = [
      ...
      },
      {
        deviceType: 'tvos',
        deviceToken:
          '3e72a1baa92a2febd9a254cbd6584f750c70b2350af5fc9052d1d12584b738e6',
    ...
    var parsePushAdapter = new ParsePushAdapter(pushConfig);
    parsePushAdapter
      .send({ data: { alert: 'some' } }, installations)
      .then(results => {
    ...
    parsePushAdapter
      ...
      .then(results => {
        ...
        results.forEach(result => {
          ...
          expect(
            result.response.error.indexOf(
              'Can not find sender for push type ios, ',
            ),
          ).toBe(0);
        });
        done();
